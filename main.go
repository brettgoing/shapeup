...
func main() {
	...
	for _, c := range cnts {
		...
		*/
		...
		// }
		gocv.DrawContour(resized, c, -1, color.RGBA{0, 255, 0, 0}, 2)
...
func detect(pts []image.Point, imgArea float64) (string, error) {
	...
	if gocv.ContourArea(approx) < /*.001*imgArea*/ 50 || gocv.IsContourConvex(approx) {
		return "", errors.New("Contour is concave or noise")
	}
